...
This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the NewsAPI Nikola 4 API. It enables AI agents and LLMs to interact with NewsAPI Nikola 4 through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with NewsAPI Nikola 4 endpoints
...
- **NewsAPI Nikola 4 Website**: [https://newsapi.org/v2](https://newsapi.org/v2)
...
*Note: Replace `example_tool` with actual NewsAPI Nikola 4 API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/newsapi-nikola-4-mcp-server.git
   cd newsapi-nikola-4-mcp-server
   ```

2. Set your API key:
   ```bash
   export NEWSAPI_NIKOLA_4_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   NEWSAPI_NIKOLA_4_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
NEWSAPI_NIKOLA_4_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own NewsAPI Nikola 4 API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_NEWSAPI_NIKOLA_4_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their NewsAPI Nikola 4 API key
2. Server uses client's API key to call NewsAPI Nikola 4 API
3. No payment required
4. Client pays NewsAPI Nikola 4 directly
...
6. Server uses its INTERNAL NewsAPI Nikola 4 API key to call the API
...
```bash
# Required
NEWSAPI_NIKOLA_4_API_KEY=your_internal_newsapi-nikola-4_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has NewsAPI Nikola 4 API key? → Mode 1 (Authenticated)
...
1. Create API client functions for NewsAPI Nikola 4 endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/newsapi-nikola-4-mcp-server",
  "mcp_server": {
    "name": "newsapi-nikola-4-mcp",
...
- `NEWSAPI_NIKOLA_4_API_KEY`: Your NewsAPI Nikola 4 API key (required)
## Troubleshooting
