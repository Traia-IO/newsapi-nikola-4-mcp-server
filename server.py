#!/usr/bin/env python3
"""
NewsAPI Nikola 4 MCP Server - FastMCP with D402 Transport Wrapper
...
- NEWSAPI_NIKOLA_4_API_KEY: Server's internal API key (for paid requests)
...
logger = logging.getLogger('newsapi-nikola-4_mcp')
...
API_KEY = os.getenv("NEWSAPI_NIKOLA_4_API_KEY")
if not API_KEY:
    logger.warning(f"⚠️  NEWSAPI_NIKOLA_4_API_KEY not set - payment required for all requests")

logger.info("="*80)
logger.info(f"NewsAPI Nikola 4 MCP Server (FastMCP + D402 Wrapper)")
...
mcp = FastMCP("NewsAPI Nikola 4 MCP Server", host="0.0.0.0")
...
# - NEWSAPI_NIKOLA_4_API_KEY: Server's internal API key for payment mode
#
...
async def search_everything(
    ...
    Args:
        ...
        from_: Start date (ISO 8601 format: YYYY-MM-DD)
        to_: End date (ISO 8601 format: YYYY-MM-DD)
    ...
    Example Usage:
        await search_everything(context, q="example_value")
    """
...
async def get_top_headlines(
    ...
    Example Usage:
        await get_top_headlines(context)
    """
    # Payment already verified by @require_payment_for_tool decorator
    # Get API key using helper (handles request.state fallback)
    api_key = get_active_api_key(context)
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="newsapi-nikola-4-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "newsapi-nikola-4-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting NewsAPI Nikola 4 MCP Server")
    logger.info("="*80)
