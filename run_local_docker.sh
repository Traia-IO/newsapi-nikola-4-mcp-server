...
# Script to build and run the NewsAPI Nikola 4 MCP Server locally in Docker
...
IMAGE_NAME="newsapi-nikola-4-mcp-server"
CONTAINER_NAME="newsapi-nikola-4-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running NewsAPI Nikola 4 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# NewsAPI Nikola 4 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO

# API Authentication (server's internal key for payment mode)
NEWSAPI_NIKOLA_4_API_KEY=
...
if ! grep -q "^NEWSAPI_NIKOLA_4_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your NewsAPI Nikola 4 API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "NEWSAPI_NIKOLA_4_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^NEWSAPI_NIKOLA_4_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^NEWSAPI_NIKOLA_4_API_KEY=.*|NEWSAPI_NIKOLA_4_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^NEWSAPI_NIKOLA_4_API_KEY=.*|NEWSAPI_NIKOLA_4_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "NEWSAPI_NIKOLA_4_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set NEWSAPI_NIKOLA_4_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ NEWSAPI_NIKOLA_4_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ NewsAPI Nikola 4 MCP Server is running!${NC}"
echo
